...
- $X \subseteq \mathbb{R}^n$ 是 **约束集合** 或 **可行域feasible set**，可以用**约束函数restrain function** $c_i(x)$表示：$ X = \left\{ x \in \mathbb{R}^n \ \middle| \ 
  \begin{aligned}
...
注：半定规划的目标函数和约束函数都是线性的，但是可行域是**非线性集合约束**，因为集合 ${X∣X⪰0}\{ X \mid X \succeq 0 \}{X∣X⪰0} $是一个**凸锥**，但不是线性子空间。所以半定规划是NP
